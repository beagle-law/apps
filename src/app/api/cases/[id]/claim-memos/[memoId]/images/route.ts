...
const MAX_SIZE = 5 * 1024 * 1024; // 5MB（スクリーンショット等の画像添付、v14）
const ALLOWED_MIME_TYPES = ["image/png", "image/jpeg", "image/gif", "image/webp"];

// 主張予定メモへのスクリーンショット等の画像アップロード。実ファイルはVercel Blobに保存する。
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string; memoId: string }> }) {
  ...
  const blob = await put(`claim-memos/${memoId}/${Date.now()}${ext}`, file, { access: "private" });

  await prisma.claimMemoImage.create({
    data: {
      claimMemoId: memoId,
      blobUrl: blob.url,
  ...
  return NextResponse.json(serializeCase(c!), { status: 201 });
}
