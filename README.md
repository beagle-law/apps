...
**現在のバージョン：v12**（要件定義書v12準拠。請求書機能を「案件」から「顧客」への紐づけに全面変更）
...
- **請求書機能を「案件」から「顧客」への紐づけに全面変更**：請求書作成・請求書履歴を案件詳細から顧客詳細に移動しました。1顧客の複数案件をまたいでタイムチャージ・実費を合算して請求書を作成できます。実費の追加・入力自体は引き続き案件詳細に残しています。
- **顧客詳細に「実費履歴」を新設**：その顧客の全案件の実費を横断表示し、行ごとの「請求」チェックボックス（`Expense.checkedForBilling`）・一括選択・実費合計/チェック済み合計を表示します。
- **請求書作成（顧客詳細）**：発行日・支払期限・宛先（自由編集、空欄で顧客名）・敬称・区分・備考に対応。区分の種別で「実費」を選ぶと、実費履歴でチェック済みの実費合計額が自動で項目に転記されます。「タイムチャージから計算して追加」は、その顧客の全案件の未請求タイムチャージを合算します。
- **顧客詳細に「請求書履歴」を新設**：入金・未入金を問わずすべての請求書を表示し、再ダウンロード・入金済み切替・削除ができます。
- **別紙実費一覧の列を「日付／金額／目的／経路」に変更**。実費のチェックが1件もない場合は別紙自体を生成しません。
- **請求管理（月次一覧）の遷移先**を顧客詳細に変更（`clientId`のない過去データのみ案件詳細にフォールバック）。
- **顧客番号の表示を一覧・詳細とも非表示化**（データ・編集ロジックは残置）。

> ⚠️ v12データ移行：`Invoice.caseId`は非推奨のフォールバックとして残しており、`clientId`のない過去の請求書（案件に顧客が紐づいていなかったもの）は顧客詳細の請求書履歴には表示されません。請求管理（月次一覧）からは引き続き参照・操作できます。実費の請求チェック（`checkedForBilling`）には排他制御を実装していません（7章の未確定事項）。複数人が同時に同じ顧客の実費を編集する運用がある場合はご注意ください。
